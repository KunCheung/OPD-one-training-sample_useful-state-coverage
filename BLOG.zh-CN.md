...
我更愿意把它当成一个 working mental model：如果 On-Policy Distillation 的监督发生在 Student 自己访问到的 state 上，那么讨论“训练数据好不好”时，除了看输入 query，也应该看这些 query 最终把 Student 带到了哪里。
...
两篇论文放在一起，会留下一个很自然的问题：

> 如果少量 query 也能诱导出大量 states，而有效蒸馏又依赖 Student 对 Teacher 信号的可利用性，那么 OPD 中真正有价值的数据到底是什么？
...
OPD 的过程更动态。Student 先从当前 policy 自己 rollout：
...
所以，一个 query 更像是进入 state space 的入口：
...
这件事看起来只是换了一种描述，但它确实改变了我们理解“数据量”的方式。

同一个 query 可以产生很多 rollout；每条 rollout 又包含很多不同 prefix。只要 Student 的采样还有随机性，同一个 query 就可能把模型带到不同的局部状态。
...
我觉得这篇论文真正重要的地方，在于它提示我们：

> **State-space coverage 可能比 query count 更接近 OPD 的有效训练规模。**

Query 更像一个 seed，真正持续产生监督机会的是 Student 后续访问到的 states。

---

## One training example is not one training state

这里保留一个我觉得很有必要的对比：**one training example is not one training state**。

论文进一步测试了一些 content-light template，甚至 off-domain 的 WildChat query。某些看起来和数学任务关系不大的 seed，仍然可以接近真实数学 query 的 OPD 效果。

更合理的理解是：**query 的文本语义和它最终诱导出的 state distribution，是两个不同层面的对象。**

一个很开放的 seed 可能让已经具备数学 reasoning 能力的 Student 自己展开推理、自我纠正、探索替代分支和中间验证。只要 rollout 过程中访问到了大量与目标能力相关的 states，Teacher 就仍然可以在这些 states 上提供有效监督。

因此，数据选择里一个更值得问的问题是：

> **这条 query 会让当前 Student 去哪里？**

这是 State Coverage 很有价值的地方。它把“数据覆盖”从 query space 往 model-induced state space 推了一步。

接下来真正有意思的是：不同 state 的学习价值可能相差很大。
...
这三个 state 都会贡献 raw coverage，但对应的 learning opportunity 显然不同。

所以我会把 State Coverage 看成第一层：先知道 Student 到了哪里；第二层再去判断这些 state 对当前 Student 有多大训练价值。
...
这意味着 Teacher–Student divergence 只能描述学习机会的一部分。

一个大 gap 可能意味着 Teacher 知道 Student 不知道的东西；也可能意味着两者局部的 reasoning distribution 差得太远，dense supervision 很难被 Student 吸收。
...
这两个维度共同决定一个 state 是否值得训练。

---

## What I mean by a “Useful State”

把前面的观察放在一起，我现在会把 Useful State 理解成一种 **student-dependent learning opportunity**。

它的价值取决于当前 Student、Teacher、目标任务，以及已经覆盖过哪些 state regions。

同一个 state，对一个弱 Student 可能非常有价值；对已经掌握这个能力的 Student，价值会明显下降。换一个 Teacher，它的价值也可能变化。
...
这里的 $f$ 只是一个占位符。真正值得实验的问题，是这些 signal 里哪些能够稳定预测 learning gain，以及它们应该作为 gate、ranking signal，还是需要联合建模。

例如，一个明显 off-task、或者 Teacher 在这里不可靠的 state，即使 Teacher-Student gap 很大，也很难成为高价值训练点。通过基本可靠性和相关性过滤后，再比较 Student 还有多少没学会的东西，以及这个区域有多新，可能更合理。
...
另一半更实际：**在真正开始大规模 OPD 之前，我们怎么知道一条 query 会产生什么样的 states？**

State 由 query 和当前 Student 一起产生：

$$ s \sim P(s \mid q, \pi_S) $$

因此，query value 最自然的估计方式之一，是先做一次很小的 **pilot rollout**。
...
这里有一个实际上的 trade-off：pilot rollout 本身也需要 Student sampling 和 Teacher inference。这个方法真正有价值的前提，是少量 rollout 就能以较低成本预测 query 在完整 OPD 中的价值。
...
即使我们能给每个 state 一个合理的 utility estimate，数据选择仍然需要考虑 coverage，因为重复访问同一种 state 的边际价值会下降。

如果一个 query 让 Student 一百次进入几乎相同的几何证明 state，那么第 100 次的新增价值显然远低于第一次。
...
这套视角带来的一个变化是：**“高质量 query”变成了一个条件性的概念。**

传统数据筛选经常会给一条样本打静态标签：难度、正确性、领域、质量分数、是否重复。

OPD 中，一条 query 的价值更像：

$$ \mathrm{Quality}\!\left(q \mid \pi_S, \pi_T, Q_{\mathrm{selected}}, \mathcal{T}\right) $$

同一条 query，在不同训练阶段可能有完全不同的价值。

训练初期，它可能不断诱导出 Student 不会但 Teacher 能教的 state。训练一段时间后，这些 region 已经被吸收，Teacher-Student gap 降低，再继续采样的收益也会下降。
...
在这个视角下，OPD 的数据选择越来越像 **state-space curriculum learning**：随着 Student 改变，下一阶段值得探索的区域也跟着改变。

---

## From query selection to state-space exploration

如果已经选了一组 query $Q$，新 query $q$ 的价值可以看它带来了多少新的 useful coverage：
...
这时，OPD 数据选择开始更像一个 exploration problem。
...
因此 Useful State Coverage 也可以被理解成一个更一般的 on-policy learning 问题：

> **模型到底经历了哪些对当前能力边界有帮助的状态？**

---

## What would convince me this idea is useful?

现在这个方向最大的风险，是它可能变成一个听起来合理但很难证伪的概念。

所以我觉得最重要的是做几个非常直接的实验。

第一个实验，是验证 **Raw State Coverage 的解释能力到底有多强**。找一些 state coverage 相近、但 Teacher-Student gap、compatibility 或 task relevance 明显不同的 query，看看 downstream OPD gain 是否明显不同。
...
第三个实验，是做数据选择。在相同 query budget、Student rollout tokens 和 Teacher inference tokens 下，对比 Random、Semantic Diversity、Raw State Coverage、Gap-only 和 Useful-State-aware Selection。

如果 raw State Coverage 已经足够解释性能，额外信号又无法稳定提升预测或选择效果，那么这个方向的增量价值就会比较有限。

如果两组 query 的 raw coverage 很接近，而“Student 是否真的能从这些 state 学到东西”可以稳定解释最终差异，那么 Useful State Coverage 就有了更扎实的研究意义。

---

## Closing thoughts

*One Training Example* 让我重新考虑 OPD 里的“训练数据”到底是什么。

Query 显然很重要，但它更像是进入 state space 的入口。真正承载 dense supervision 的，是 Student 自己 rollout 时不断访问到的 states。

State Coverage 把这个问题往前推进了一大步：除了问“有多少 query”，我们还可以问“模型到底去了多少地方”。

如果不同 state 的学习价值差异很大，那么下一步自然是继续问：
...
如果这个方向成立，那么 OPD 的数据工程可能会逐渐变成一种动态的 state-space exploration：Student 当前在哪些区域还有明显 learning opportunity，就把更多预算放到那里；吸收之后，再去寻找下一批值得探索的 states。
